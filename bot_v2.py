...
async def get_group_languages(base):
    docs = await movies.find({"enabled": True, "title_normalized": base["title_normalized"], "year": base.get("year")}, {"language": 1}).to_list(length=SEARCH_LIMIT)
...
async def add_existing(event):
    ...
    if len(parts) != 5: await event.respond("Usage: `/add <message_id> | title | year | language | quality`"); return
...
async def movie_search(event):
    ...
    count = len(groups)
    await event.respond(f"🎬 **{count} movie{'s' if count != 1 else ''} found**\n\nSelect your movie:", buttons=[[Button.inline(f"{clean_text(g.get('title') or 'Unknown title')} ({g.get('year') or 'Year unknown'}) • {g.get('file_count', 0)} file{'s' if g.get('file_count', 0) != 1 else ''}"[:64], data=f"movie:{g['representative_id']}")] for g in groups])
...
async def choose_language(event):
    ...
    docs = await movies.find({"enabled": True, "title_normalized": base["title_normalized"], "year": base.get("year"), "language": language}, {"quality": 1}).to_list(length=SEARCH_LIMIT)
...
async def choose_quality(event):
    ...
    doc = await movies.find_one({"enabled": True, "title_normalized": base["title_normalized"], "year": base.get("year"), "language": language, "quality": quality})
    ...
    try:
        ...
        year = base.get("year") or "Year unknown"
        await event.edit(f"✅ **{base['title']} ({year})**\n🌐 {language} • 📺 {quality}\n\n📁 File sent above.")
    except (MessageIdInvalidError, ValueError): log.exception("Could not send stored media"); await event.edit("⚠️ I could not send that file. The stored channel message may be unavailable.")
    except Exception: log.exception("Unexpected send error"); await event.edit("⚠️ Something went wrong while sending the file. Please try again later.")
    await event.answer()
